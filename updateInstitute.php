...
<div class="card">
    ...
    <?php 
          include("includes/db.php");
          if(isset($_GET['updateInstitute']))
          {
            $edit_ID = $_GET['updateInstitute'];
            $get_institute = "select * from institute where Institute_id='$edit_ID'";
            $run_institute = mysqli_query($con,$get_institute);
            $row_institute=mysqli_fetch_array($run_institute);
    ...
    <div class="card-body px-lg-5 pt-0">
      ...
      <form class="text-center"  style="color: #757575;" method="post">
        ...
        <!-- Email -->
        <div class="md-form">
          <input type="text" name="Institute" id="UserEmail" value="<?php echo $Name; ?>" class="form-control">
          <label for="Institute">Institute Name</label>
        </div>
        <button class="btn btn-outline-info btn-rounded btn-block my-4 waves-effect z-depth-0" name="updateInstitute" style="border-radius: 100px;" type="submit">Update</button>
      </form>
    </div>
  </div>
</div>
<?php         
     if(isset($_POST['updateInstitute']))
  {
    ...
    $update_institute = "update institute set I_Name='$Institute' where Institute_id='$Institute_ID'";
    $run_update = mysqli_query($con,$update_institute) or die("Error: ".mysqli_error($con));
    if($run_update)
    {
        echo "<script>alert('Institute has Been Updated')</script>";
